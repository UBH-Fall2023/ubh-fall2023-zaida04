...
export default function MainPage(props: Props) {
  ...
  return (
    <div className="h-max flex flex-col items-center justify-center">
      ...
      <Button disabled={selectedOrders.length === 0} onClick={claimOrder}>I'm Ready</Button>
      <div className={`h-4/6 mt-3 w-5/6 ${query.isLoading || query.isError ? 'text-center' : null}`}>
        {query.isLoading && <h1>Loading...</h1>}
        {query.isError && <h1>Oops! An error occured, please try refreshing the page.</h1>}
        {query.data && query.data.data && query.data.data.length > 0
          ...
          <Table className="min-w-full mt-2 border border-gray-300">
            ...
            <TableBody>
              {query.data.data.map((orderInfo, idx) => {
                return (
                  <TableRow key={idx}>
                    <TableCell className="font-medium">
                      <div className="flex gap-2">
                        <Checkbox onCheckedChange={(c) => selectOrder(c, orderInfo.id)} className="my-auto"/>
                        <p>Claim</p>
                      </div>
                    </TableCell>
        ...
        : query.isLoading ? null : <span>No orders to be fulfilled...(yet!)</span>
        }
      </div>
    </div>
  )
