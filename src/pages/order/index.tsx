...
const queryAtom = atomWithLocation();
  ...
  return (
    <>
      <div className="flex flex-col h-screen bg-secondary">
        ...
        <div className="flex h-fit py-4 gap-x-6 overflow-x-scroll px-6">
          {stores.map((store) => (
            <Button
              onClick={() => {
              ...
              className={cn([
                ...
                query.searchParams?.get("store") === store.value
        ...
        <div className="h-3/5 flex bg-white border-t-2 w-full">
          <div className="w-48 border-r-2 h-full p-2 flex flex-col items-center gap-y-2">
            ...
            {stores
              .find((store) => store.value === query.searchParams?.get("store"))
              ?.dishTypes.map((item) => (
                <Button
                  onClick={() => {
                    const entries = [
          ...
          <div className=" flex flex-col  w-full ">
            ...
            <div className="flex w-full p-4 gap-x-5 overflow-y-scroll h-full">
              {run(() => {
                ...
                return stores
                  .find(
                    (store) => store.value == query.searchParams?.get("store"),
                  )
                  ?.items.filter(
